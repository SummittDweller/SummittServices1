...
function SummittServices1_menu_local_task($link, $active = FALSE) {
  $active_class = "";
  if ($active) {
    $active_class .= "active ";
  }
 ...
 *   A descriptive title of the feed.
  */
function SummittServices1_feed_icon($url, $title) {
 ...
 */
function SummittServices1_preprocess_comment_wrapper(&$vars) {
 ...
 */
function SummittServices1_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb art-postcontent">'. implode(' | ', $breadcrumb) .'</div>';
  }
}

function SummittServices1_service_links_node_format($links) {
 ...
 * @ingroup themeable
 */
function SummittServices1_button($element) {
...
*/
function SummittServices1_img_assist_page($content, $attributes = NULL) {
 ...
 * @ingroup themeable
 */
function SummittServices1_node_preview($node) {
